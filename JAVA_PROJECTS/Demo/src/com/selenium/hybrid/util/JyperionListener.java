...
public class JyperionListener implements ITestListener {
		...
		if (throwable != null) {
			...
			this.nbExceptions++;
			 ...
			 Chunk imdb = new Chunk("[SCREEN SHOT]", new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE));
		        imdb.setAction(new PdfAction(System.getProperty("user.dir")+"\\screenshots\\"+Keywords.fileName));
		        Paragraph  excep = new Paragraph(
			            throwable.toString());
			        excep.add(imdb);
			cell = new PdfPCell(excep);
	...
	public void onStart(ITestContext context) {
		log("onStart("+context+")");
		try {
			PdfWriter.getInstance(this.document, new FileOutputStream(System.getProperty("user.dir")+"//Report//"+context.getName()+".pdf"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		this.document.open();
